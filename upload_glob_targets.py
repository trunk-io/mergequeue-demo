...
def main():
    ...
    required_env = ["API_TOKEN", "REPOSITORY", "TARGET_BRANCH", "PR_NUMBER", "PR_SHA"]
    ...
    if IMPACTS_ALL_DETECTED == "true":
        ...
        # Read impacted targets from JSON file
        impacted_targets_file = "impacted_targets_json_tmp"
        try:
            with open(impacted_targets_file, "r") as f:
                impacted_targets = json.load(f)
                # Filter out empty strings
                impacted_targets = [target for target in impacted_targets if target]
        except FileNotFoundError:
            eprint(f"Impacted targets file not found: {impacted_targets_file}")
            sys.exit(2)
        except json.JSONDecodeError as e:
            eprint(f"Error parsing JSON from {impacted_targets_file}: {e}")
            sys.exit(2)
    ...
    if http_status_code == 200:
        ...
        if http_status_code == 401:
            ...
            elif ACTOR and ACTOR.endswith("[bot]"):
